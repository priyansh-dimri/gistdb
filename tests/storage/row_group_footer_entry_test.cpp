...
TEST(RowGroupFooterEntryTest, ConstructionStoresAllFields) {
  RowGroupFooterEntry entry(3, kRowGroupSize, PageRange{10, 2},
                            MakeTwoColumnFixtures());
...
TEST(RowGroupFooterEntryTest, ColumnAccessReturnsCorrectVariantAlternative) {
  RowGroupFooterEntry entry(1, kRowGroupSize, PageRange{0, 1},
                            MakeTwoColumnFixtures());

  ASSERT_TRUE(std::holds_alternative<FixedWidthColumnFooterEntry<std::int32_t>>(
      entry.Column(0)));
  ASSERT_TRUE(
      std::holds_alternative<VarcharColumnFooterEntry>(entry.Column(1)));

  const auto &int_entry =
      std::get<FixedWidthColumnFooterEntry<std::int32_t>>(entry.Column(0));
  EXPECT_EQ(int_entry.Zone().Min(), 1);

  const auto &varchar_entry =
      std::get<VarcharColumnFooterEntry>(entry.Column(1));
...
TEST(RowGroupFooterEntryTest, FloatColumnVariantAlsoWorks) {
  ...
  auto float_footer =
      FixedWidthColumnFooterEntry<float>::Build(float_column, PageRange{0, 1});

  RowGroupFooterEntry entry(1, 1, PageRange{0, 1},
                            {ColumnFooterEntry{float_footer}});

  ASSERT_TRUE(std::holds_alternative<FixedWidthColumnFooterEntry<float>>(
      entry.Column(0)));
  EXPECT_FLOAT_EQ(std::get<FixedWidthColumnFooterEntry<float>>(entry.Column(0))
                      .Zone()
                      .Min(),
                  9.5F);
}
