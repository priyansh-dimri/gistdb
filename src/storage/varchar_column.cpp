...
namespace gistdb::storage
{
    ...
    bool VarcharColumn::IsNull(std::size_t index) const { return validity_.IsNull(index); }
    bool VarcharColumn::IsValid(std::size_t index) const { return validity_.IsValid(index); }

    std::size_t VarcharColumn::Size() const { return offsets_.size() - 1; }

    const std::uint8_t *VarcharColumn::DataBuffer() const { return data_buffer_.data(); }
    std::size_t VarcharColumn::DataBufferSize() const { return data_buffer_.size(); }

    const std::int32_t *VarcharColumn::Offsets() const { return offsets_.data(); }
    std::size_t VarcharColumn::NumOffsets() const { return offsets_.size(); }

    const ValidityBitmap &VarcharColumn::Validity() const { return validity_; }
