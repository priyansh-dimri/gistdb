...
class VarcharColumn {
 public:
  ...
  [[nodiscard]] std::string_view GetValue(std::size_t index) const;  // pre: !IsNull(index)
  ...
  [[nodiscard]] const std::uint8_t* DataBuffer() const;
  [[nodiscard]] std::size_t DataBufferSize() const;

  [[nodiscard]] const std::int32_t* Offsets() const;  // size() == Size() + 1, Offsets()[0] == 0
  [[nodiscard]] std::size_t NumOffsets() const;
