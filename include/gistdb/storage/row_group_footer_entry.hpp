...
public:
  ...
  std::uint32_t RowCount() const { return row_count_; }
  PageRange ValidityBitmapRegion() const { return validity_bitmap_region_; }

  std::size_t NumColumns() const { return columns_.size(); }
  const ColumnFooterEntry &
  Column(std::size_t ordinal) const; // pre: ordinal < NumColumns()
